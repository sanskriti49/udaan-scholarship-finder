	...
	return (
      ...
      `}</style>
			...
			<div className="min-h-screen bg-white text-gray-900 font-pangea">
				{/* ════════ HERO ════════ */}
				<section className="relative overflow-hidden bg-[#F6FAF1] border-b border-[#DDECCB] py-16 px-6">
					<div className="absolute -top-24 -right-24 w-80 h-80 bg-[#5AAD1F]/[.04] rounded-full blur-3xl pointer-events-none" />
					<div className="absolute -bottom-20 -left-20 w-64 h-64 bg-[#5AAD1F]/[.04] rounded-full blur-3xl pointer-events-none" />
					...
					<div className="max-w-5xl mx-auto">
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
							{GUIDES.map((g, i) => (
								<div
									key={i}
									className="relative border border-gray-200/80 hover:border-[#C0DD97] hover:shadow-lg rounded-2xl p-6 cursor-pointer transition-all duration-300 flex flex-col group bg-white fade-up"
									style={{ animationDelay: `${i * 80}ms` }}
								>
									<div
										className={`w-10 h-10 rounded-xl ${g.bg} flex items-center justify-center text-xl mb-4 mt-1 group-hover:scale-110 transition-transform duration-300`}
									>
										{g.icon}
									</div>
									<h3 className="text-[17px] font-semibold text-gray-900 mb-1.5 leading-snug">
										{g.title}
									</h3>
									<p className="text-[13.5px] tracking-wide text-gray-500 leading-relaxed flex-1">
										{g.description}
									</p>
									<div className="mt-5 flex items-center gap-1 text-[#5AAD1F] text-[14px] font-bold group-hover:gap-2.5 transition-all duration-200">
										Read guide
										<svg
				...
				<section className="py-14 px-6">
					<div className="max-w-5xl mx-auto">
						...
						<div className="grid grid-cols-1 lg:grid-cols-[250px_1fr] gap-6">
							{/* ── Progress Ring ── */}
							<div className="relative overflow-hidden bg-gradient-to-br from-[#F6FAF1] to-[#EAF3DE] border border-[#DDECCB] rounded-2xl p-6 flex flex-col items-center justify-center text-center min-h-[290px]">
								{/* Confetti layer */}
								{particles.map((p) => (
									<div
										key={p.id}
								...
								<div className={`relative ${allDone ? "ring-pulse" : ""}`}>
									...
									<div className="absolute inset-0 flex flex-col items-center justify-center">
										...
										<span className="text-[9px] font-bold tracking-[.2em] text-gray-400 uppercase mt-1">
											Ready
										</span>
									</div>
								</div>
								...
								<div className="mt-5">
									...
									<p className="text-[12px] text-gray-500 mt-0.5 leading-relaxed max-w-[180px]">
										{allDone
							...
							<div className="bg-white border border-gray-200/80 rounded-2xl p-7 shadow-sm">
								<div className="flex items-center justify-between mb-5">
									...
									{checkedDocs.length > 0 && (
										<button
											onClick={resetDocs}
											className="text-[13px] text-gray-400 hover:text-red-500 transition-colors cursor-pointer font-medium"
										>
											Reset all
										</button>
									)}
									...
									{DOCUMENTS.map((doc, idx) => {
										...
										return (
											<button
												...
												className={`flex items-center gap-3 p-3.5 rounded-xl border transition-all duration-200 text-left cursor-pointer group/chip ${
													isChecked
											...
											>
												...
												{/* Label */}
												<span
													className={`text-[13.5px] font-inter font-medium transition-all duration-200 truncate ${
					...
					<div className="max-w-5xl mx-auto">
						...
						<div className="grid grid-cols-1 md:grid-cols-3 gap-4">
							{EDU_RESOURCES.map((res, i) => (
								<div
									key={i}
									className="group border border-gray-200/80 hover:border-[#C0DD97] hover:shadow-lg rounded-2xl p-6 cursor-pointer transition-all duration-300 bg-white fade-up"
									style={{ animationDelay: `${i * 100}ms` }}
								>
									<div
										className={`w-11 h-11 rounded-xl ${res.bg} flex items-center justify-center text-lg mb-4 group-hover:scale-110 transition-transform duration-300`}
									>
										{res.icon}
									</div>
									<h4 className="text-[16.5px] font-semibold text-gray-900 group-hover:text-[#5AAD1F] transition-colors mb-1.5">
										{res.title}
									</h4>
									<p className="text-[13.5px] text-gray-500 leading-relaxed">
										{res.desc}
									</p>
									<div className="mt-4 flex items-center gap-1 text-[#5AAD1F] text-[13px] font-bold opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0 transition-all duration-200">
										Explore
										<svg
											className="w-3.5 h-3.5 group-hover:translate-x-0.5 transition-transform"
											fill="none"
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
							{PORTALS.map((portal, i) => (
								<a
									key={i}
									href="#"
									className="group relative overflow-hidden bg-white border border-gray-200/80 hover:border-[#C0DD97] hover:shadow-lg rounded-2xl p-5 flex items-center justify-between transition-all duration-300"
								>
									<div
										className={`absolute left-0 top-0 w-1 h-full ${portal.color} rounded-l-2xl group-hover:w-1.5 transition-all duration-300`}
									/>
									<span className="font-jakarta font-bold text-[14px] text-gray-800 pl-3 group-hover:text-[#5AAD1F] transition-colors duration-200">
										{portal.name}
									</span>
									<svg
				...
				<section className="py-10 px-6">
					<div className="max-w-5xl mx-auto bg-[#EAF3DE] border border-[#C0DD97] rounded-2xl px-8 py-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
						...
						<div className="flex flex-wrap gap-3 shrink-0">
							<Link
								to="/eligibility"
								className="inline-flex items-center justify-center bg-[#5AAD1F] text-white text-[15px] font-semibold px-6 py-2.5 rounded-full hover:bg-[#4A9A18] active:bg-[#3D8813] transition-all duration-200 shadow-sm hover:shadow-md active:scale-[0.98]"
							>
								Check eligibility
							</Link>
							<Link
								to="/scholarships"
								className="inline-flex items-center justify-center bg-white border border-[#C0DD97] text-[#27500A] text-[15px] font-semibold px-6 py-2.5 rounded-full hover:bg-[#F6FAF1] hover:border-[#5AAD1F] transition-all duration-200"
							>
								Browse scholarships
							</Link>
						</div>
