...
import { scanLinkOrContent, getOfficialRegistry } from "../services/verifyService";
...
export default function TrustShield() {
  ...
  useEffect(() => {
    getOfficialRegistry()
      ...
      .catch((err) => {
        console.warn("Could not load remote registry, using local fallback:", err?.message);
      })
  ...
  const handleAnalyze = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        if (res.result.verdict === "VERIFIED_OFFICIAL" || res.result.verdict === "VERIFIED_CSR") {
          toast.success("Verification complete: Official or recognized entity detected.");
        } else if (res.result.verdict === "HIGH_RISK_SUSPICIOUS") {
          toast.error("High risk detected! Review the warning indicators below.");
        } else {
          toast.info("Analysis complete: Unverified third-party link.");
        }
      }
    } catch (err) {
      toast.error(err.response?.data?.message || "Failed to analyze link. Please check your connection.");
    } finally {
      setAnalyzing(false);
    }
  };
      ...
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        <div className="text-center max-w-3xl mx-auto">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-serif text-slate-900 leading-tight">
            Anti-Scam & <span className="italic text-emerald-800 font-normal">Trust Shield</span>
          </h1>
          <p className="text-sm sm:text-base text-slate-600 mt-3 font-normal leading-relaxed">
            Verify external scholarship links, WhatsApp forwards, and SMS circulars before sharing personal details. Official government scholarships are legally 100% free to apply.
          </p>
        </div>
        ...
        <div className="bg-white border border-slate-200/90 rounded-3xl p-6 sm:p-8 shadow-2xs space-y-6">
            ...
            <div className="flex flex-wrap gap-2">
              {presets.map((p, idx) => (
                <button
                  ...
                  className="cursor-pointer text-xs font-medium px-3 py-1.5 rounded-xl border border-slate-200 bg-[#FAF9F6] hover:bg-emerald-50/50 hover:border-emerald-300 text-slate-700 transition flex items-center gap-1.5"
                >
          ...
          <form onSubmit={handleAnalyze} className="space-y-4">
            {activeTab === "url" ? (
              <div className="relative">
                ...
                />
                <Globe size={18} className="absolute right-4 top-3.5 text-slate-400" />
              </div>
            ) : (
              <div className="relative">
                <textarea
            ...
            <div className="flex items-center justify-between pt-2">
              <div className="text-xs text-slate-500 flex items-center gap-1.5">
                ...
                <span>Government scholarships never ask for application fees or UPI transfers.</span>
              </div>
              <button
                type="submit"
                disabled={analyzing}
          ...
          {result && (() => {
            ...
            return (
              <div className={`mt-6 rounded-2xl border p-5 sm:p-6 space-y-5 transition-all ${details.cardBg}`}>
