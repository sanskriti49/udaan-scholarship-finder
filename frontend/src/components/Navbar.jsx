	...
	return (
		<>
			...
			>
					...
					<div className="hidden md:flex items-center bg-gray-100/80 border border-gray-200/50 rounded-full p-1 gap-1">
						{navLinks.map((link) => (
							<NavLink
								to={link.path}
								className={({ isActive }) =>
									`px-4 py-1.5 text-sm rounded-full transition-all duration-200 ease-out whitespace-nowrap ${
										isActive
				...
				<div
					className={`fixed top-[84px] left-0 right-0 bg-white border-b border-gray-100 shadow-lg transition-all duration-300 md:hidden ${
						isOpen
					...
					<div className="px-6 py-5 flex flex-col gap-2">
						{navLinks.map((link) => (
							<NavLink
								...
								to={link.path}
								className={({ isActive }) =>
									`px-4 py-2.5 text-base rounded-lg transition-all ${
										isActive
